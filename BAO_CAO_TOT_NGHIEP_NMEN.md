...
sequenceDiagram
    ...
    Khach->>FE: Nhập email, password & Click Đăng nhập
    FE->>BE: POST /api/auth/login {email, password}
    BE->>DB: SELECT * FROM users WHERE email = ?
    DB-->>BE: Trả về thông tin người dùng
    alt Email không tồn tại hoặc mật khẩu sai
        BE-->>FE: HTTP 401: Email hoặc mật khẩu không đúng
        FE-->>Khach: Hiển thị thông báo lỗi đăng nhập
    else Tài khoản bị khóa (status = inactive)
        BE-->>FE: HTTP 403: Tài khoản của bạn đã bị khóa
        FE-->>Khach: Thông báo tài khoản bị khóa
    else Đăng nhập thành công
        BE->>BE: So sánh mật khẩu băm (bcrypt.compare)
        BE->>BE: Ký token JWT (jwt.sign) với payload {id, email, role}
        BE-->>FE: HTTP 200: Trả về Token JWT + Thông tin User
        FE->>FE: Lưu Token vào localStorage (nmen_token)
        FE-->>Khach: Điều hướng về Trang chủ / Dashboard cá nhân
    end
```
...
sequenceDiagram
    ...
    Khach->>FE: Click xem chi tiết sản phẩm
    FE->>BE: GET /api/products/:slug
    BE->>DB: SELECT * FROM products WHERE slug = ?
    DB-->>BE: Thông tin sản phẩm gốc
    BE->>DB: SELECT * FROM product_images WHERE product_id = ?
    DB-->>BE: Danh sách ảnh sản phẩm
    BE->>DB: SELECT * FROM product_variants WHERE product_id = ?
    DB-->>BE: Danh sách biến thể (color_name, color_hex, size, stock)
    BE-->>FE: Gộp dữ liệu sản phẩm, ảnh và các biến thể dạng JSON
    FE->>FE: Render thông tin lên giao diện chi tiết
    Khach->>FE: Chọn Màu sắc và Kích cỡ tương ứng
    FE->>FE: Kiểm tra thuộc tính biến thể đã chọn
    alt Biến thể đó có stock = 0
        FE-->>Khach: Hiển thị trạng thái "Hết hàng" (nút Mua bị vô hiệu hóa)
    else Biến thể đó có stock > 0
        FE-->>Khach: Hiển thị trạng thái "Còn hàng" (cho phép chọn số lượng & Mua)
    end
```
...
sequenceDiagram
    ...
    participant BE as Express API Server
    participant DB as MySQL Database
    actor Sepay as Cổng thanh toán Sepay

    Khach->>FE: Điền thông tin giao hàng & Click Đặt hàng (Sepay)
    FE->>BE: POST /api/orders {items, promo_code, payment_method: 'Sepay', ...}
    BE->>DB: Khởi động giao dịch (conn.beginTransaction)
    loop Quét qua từng item trong đơn hàng
        BE->>DB: SELECT stock FROM product_variants WHERE id = ?
        DB-->>BE: Trả về tồn kho biến thể sản phẩm
        alt Tồn kho < Số lượng đặt mua
            BE->>DB: Rollback giao dịch (conn.rollback)
            BE-->>FE: HTTP 400: Sản phẩm "A" không đủ số lượng trong kho
            FE-->>Khach: Thông báo đặt hàng thất bại do hết hàng
        else Tồn kho >= Số lượng đặt
            BE->>DB: Cập nhật giảm kho: UPDATE product_variants SET stock = stock - ? WHERE id = ?
        end
    end
    BE->>DB: INSERT INTO orders (order_number, total_amount, payment_status: 'pending', ...)
    BE->>DB: INSERT INTO order_items (...)
    BE->>DB: INSERT INTO payment_transactions (order_id, status: 'pending', ...)
    BE->>DB: Commit giao dịch (conn.commit)
    BE->>BE: Sinh mã VietQR động chứa mã chuyển khoản (NM-[MÃ_ĐƠN]) và số tiền
    BE-->>FE: HTTP 201: Trả về thông tin đơn hàng và mã QR
    FE-->>Khach: Hiển thị giao diện chuyển khoản kèm mã QR ngân hàng

    Note over Khach, Sepay: Khách hàng quét mã VietQR và thực hiện chuyển khoản bằng Mobile Banking
    Sepay->>BE: POST /api/sepay/webhook {referenceNumber, amountIn, transactionContent}
    BE->>BE: Xác thực Authorization Bearer token của webhook
    alt Token không hợp lệ
        BE-->>Sepay: HTTP 401 Unauthorized
    else Token hợp lệ
        BE->>DB: SELECT id FROM payment_transactions WHERE reference_number = ? (Chống trùng lặp)
        DB-->>BE: Trả về bản ghi giao dịch
        alt Giao dịch đã được xử lý từ trước (Idempotency)
            BE-->>Sepay: HTTP 200: Transaction already processed
        else Giao dịch mới
            BE->>BE: Regex trích xuất mã đơn hàng từ transactionContent
            BE->>DB: SELECT total_amount, payment_status FROM orders WHERE order_number = ?
            DB-->>BE: Thông tin đơn hàng
            alt Số tiền nhận được (amountIn) >= total_amount và đơn hàng đang pending
                BE->>DB: UPDATE orders SET payment_status = 'paid', status = 'confirmed' WHERE id = ?
                BE->>DB: UPDATE payment_transactions SET status = 'success', raw_payload = ? WHERE id = ?
                BE-->>Sepay: HTTP 200 OK: Webhook processed
            else Tiền không khớp hoặc đơn hàng đã xử lý trước đó
                BE->>DB: UPDATE payment_transactions SET status = 'failed' WHERE id = ?
                BE-->>Sepay: HTTP 200 OK: Webhook processed with failure state
            end
        end
    end
```
...
sequenceDiagram
    autonumber
    actor Admin as Admin / Khách hàng
    participant BE as Express API Server
    participant DB as MySQL Database

    Admin->>BE: POST /api/orders/cancel {order_number, phone, reason}
    BE->>DB: SELECT id, status FROM orders WHERE order_number = ?
    DB-->>BE: Thông tin trạng thái đơn hàng
    alt Trạng thái đơn hàng không phải 'pending'
        BE-->>Admin: HTTP 409: Đơn hàng đã xử lý, không thể hủy
    else Đơn hàng ở trạng thái 'pending'
        BE->>DB: Khởi động giao dịch (conn.beginTransaction)
        BE->>DB: UPDATE orders SET status = 'cancelled', cancelled_reason = ? WHERE id = ?
        BE->>DB: SELECT variant_id, quantity FROM order_items WHERE order_id = ?
        DB-->>BE: Danh sách sản phẩm của đơn hàng
        loop Quét qua từng item trong đơn hàng
            BE->>DB: UPDATE product_variants SET stock = stock + ? WHERE id = ?
        end
        BE->>DB: Commit giao dịch (conn.commit)
        BE-->>Admin: HTTP 200: Hủy đơn hàng thành công, kho hàng được khôi phục
    end
```
...
erDiagram
    ...
    product_variants ||--o{ order_items : "sold_in"
