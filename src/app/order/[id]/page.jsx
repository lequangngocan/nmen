...
const MOCK_ORDER = {
  ...
  address: "Julian Sterling\n1248 North Meridian St.\nApartment 4B\nPortland, OR 97201\nUnited States\n+1 (503) 555-0192",
  items: [
    {
      id: "oi1",
  ...
  return (
    <main className="pt-24 lg:pt-32 pb-24 max-w-7xl mx-auto px-6 md:px-12 text-black bg-stone-50 min-h-screen">
      ...
      <Link href="/account" className="flex items-center gap-2 mb-12 hover:opacity-60 transition-all w-max group">
        <ArrowLeft size={16} />
        <span className="font-label text-xs uppercase tracking-widest font-medium">Back to Account</span>
      </Link>
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20 border-b border-stone-200 pb-8">
        <div>
          <h1 className="font-headline text-4xl md:text-7xl font-extrabold tracking-tighter uppercase mb-6 md:mb-4">Order Details</h1>
          <div className="flex flex-wrap gap-x-8 gap-y-4">
            
            <div className="flex flex-col">
              <span className="font-label text-[10px] text-stone-500 uppercase tracking-[0.2em] mb-1">Order ID</span>
              <span className="font-headline font-bold text-lg uppercase text-black">{orderId}</span>
            </div>
            
            <div className="flex flex-col">
              <span className="font-label text-[10px] text-stone-500 uppercase tracking-[0.2em] mb-1">Placed On</span>
              <span className="font-headline font-bold text-lg text-black">{MOCK_ORDER.placedOn}</span>
            </div>
            
            <div className="flex flex-col">
              <span className="font-label text-[10px] text-stone-500 uppercase tracking-[0.2em] mb-1">Status</span>
      ...
      <section className="mb-24 bg-stone-200 p-8 md:p-16">
        <div className="relative flex justify-between items-center max-w-5xl mx-auto">
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 bg-stone-200 px-2 md:px-4">
            ...
            <span className="font-label text-[10px] font-bold uppercase tracking-widest text-black text-center">Processing</span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 bg-stone-200 px-2 md:px-4">
            ...
            <span className="font-label text-[10px] font-bold uppercase tracking-widest text-black text-center">Shipped</span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 bg-stone-200 px-2 md:px-4">
            ...
            <span className="font-label text-[10px] font-bold uppercase tracking-widest text-stone-400 text-center">Out for<br className="md:hidden"/> Delivery</span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 bg-stone-200 px-2 md:px-4">
            ...
            <span className="font-label text-[10px] font-bold uppercase tracking-widest text-stone-400 text-center">Delivered</span>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-8">
          <h3 className="font-headline font-extrabold text-2xl uppercase tracking-tighter mb-8 text-black">Shipment Contents ({MOCK_ORDER.items.length})</h3>
                ...
                <div className="flex-1 flex flex-col justify-between py-1 sm:py-2">
                  <div className="flex flex-col xl:flex-row justify-between items-start gap-4">
                    <div>
                      ...
                      <p className="font-label text-[10px] sm:text-xs text-stone-500 uppercase tracking-widest mb-1">Color: {item.color}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-4 mt-6">
                    <span className="font-label text-[10px] uppercase tracking-widest text-stone-500">Quantity: {item.qty}</span>
        ...
        <div className="lg:col-span-4 space-y-12">
          ...
          <div className="bg-white ring-1 ring-black/5 shadow-sm p-8 lg:p-10">
            <h3 className="font-headline font-extrabold text-lg uppercase tracking-tighter mb-6 text-black">Delivery Address</h3>
            <p className="font-body text-sm leading-relaxed text-stone-600 whitespace-pre-line">
              {MOCK_ORDER.address}
            </p>
            
            <div className="mt-10 pt-8 border-t border-stone-200">
              <h3 className="font-headline font-extrabold text-lg uppercase tracking-tighter mb-4 text-black">Shipping Method</h3>
              <p className="font-body text-sm text-black">{MOCK_ORDER.shippingMethod}</p>
              <p className="font-label text-[10px] text-stone-500 uppercase tracking-widest mt-1">Tracking: {MOCK_ORDER.trackingNo}</p>
            </div>
          </div>

          {/* Khung Total Data (Khung Đen sang trọng) */}
          <div className="bg-black text-white p-8 lg:p-10 shadow-lg">
            <h3 className="font-headline font-extrabold text-lg uppercase tracking-tighter mb-8">Summary</h3>
            <div className="space-y-5 mb-8">
              <div className="flex justify-between font-label text-[10px] uppercase tracking-widest text-stone-300">
                <span>Subtotal</span>
                <span>${MOCK_ORDER.subtotal.toLocaleString("en-US", {minimumFractionDigits: 2})}</span>
              </div>
              <div className="flex justify-between font-label text-[10px] uppercase tracking-widest text-stone-300">
                <span>Shipping</span>
                <span>${MOCK_ORDER.shipping.toLocaleString("en-US", {minimumFractionDigits: 2})}</span>
              </div>
              <div className="flex justify-between font-label text-[10px] uppercase tracking-widest text-stone-300">
                <span>Tax</span>
                <span>${MOCK_ORDER.tax.toLocaleString("en-US", {minimumFractionDigits: 2})}</span>
              </div>
            </div>
            <div className="pt-6 border-t border-white/20 flex justify-between items-end">
              <span className="font-headline font-extrabold text-lg uppercase tracking-widest">Total</span>
              <span className="font-headline font-extrabold text-3xl md:text-2xl xl:text-3xl">${MOCK_ORDER.total.toLocaleString("en-US", {minimumFractionDigits: 2})}</span>
            </div>
          </div>
          ...
          <div className="p-1 border border-stone-200 hover:border-black transition-colors">
            <div className="p-6 lg:p-8 bg-stone-100 flex items-center gap-4 cursor-pointer hover:bg-stone-200 transition-colors">
              ...
              <div>
                <p className="font-headline font-bold text-sm uppercase text-black">Need Assistance?</p>
                <p className="font-label text-[10px] uppercase tracking-widest text-stone-500 mt-1">Contact Concierge 24/7</p>
              </div>
            </div>
          </div>
